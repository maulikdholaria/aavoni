...
class PhotographerEdit extends React.Component {
  constructor(props) {
  	super(props);
  	this.state = {
  	  data: {
  ...
  handleSubmit = (values) => {
    ...
    values['marketCity'] = GlobalMapping['location'][values['marketCityId']-1]['marketCity'];
  	...
  	photographersApi.edit(values, response => {
    });
  }
  ...
  componentDidMount() {
    window.analytics.page('Photographer Edit');
    const usersApi = new UsersApi();
    usersApi.getLoggedInUser(response => {
      if(response.data.success == true) {
        const photographersApi = new PhotographersApi();
        photographersApi.get(this.props.match.params.id, response => {
